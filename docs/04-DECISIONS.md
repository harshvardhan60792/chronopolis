...
**Status:** accepted
**Context:** Incremental rebuilding was originally meant to include a parsing cache. However, T23 performance measurements revealed that parsing source files takes <5% of the total build time even on large repositories (e.g. 24.49s for git_read vs 0.91s for parse). Caching parsed files would add architectural complexity for negligible gains.
