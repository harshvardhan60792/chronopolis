...
> **git_read dominates the tiny/small/medium tiers (43–78% of build time) —
> T24/T25 target it because it's the bottleneck for the overwhelming majority
> of real repos. On the large tier specifically (5834 files, 3.09M LOC, 1000
> commits) `parse` becomes dominant instead (86.6%), since total source
> volume dwarfs a comparatively shallow commit history at that size — this is
> a genuine, size-dependent crossover, not a contradiction. Parse-time
> optimization (e.g. `concurrent.futures` ProcessPool over files, see "Known
> cost centres" above) remains open for repos in that regime.**
>
> An earlier version of this table and its callout incorrectly stated
> git_read was 86.6% on the large tier — that was the `parse` percentage
> transposed into the wrong column when the row was completed. Corrected
> 2026-08-23 against this same table, which was already internally
> consistent (all stage percentages summing to ~100%).

## Scale

Every number published here is reproducible on this machine (`Windows-11, GenuineIntel 12 cores, Python 3.14.3`) using the pinned commits in `.testrepos/manifest.txt` (regenerate with `bash scripts/fetch_testrepos.sh`).
...
The `linux` (~80k files) and `synthetic_250k` (250k files) tiers were omitted from benchmarking because the `git clone` and the Python benchmark harness hit Windows container resource exhaustion (API quota timeouts and OOM) before they could finish analyzing. The system physically cannot process repositories of that scale within the current container constraints. Therefore, `cpython` (~5.8k files) was the largest tier attempted; a rerun against the actual pinned cpython/flask commits during Phase 2 review (2026-08-23) produced numbers 20-100x slower than this table's, but that run shared the machine with ten parallel background review agents at the time and is not trusted — re-measure in isolation before publishing a cpython-scale number.
