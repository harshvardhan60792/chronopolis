...
def score_paths(city: dict, paths: list[str], weights: dict[str, float] = None) -> list[dict]:
    """score_all filtered to the given paths, preserving the given order.

    Previously this reimplemented ~150 lines of score_all's own scoring
    body with a different blast-radius reference distribution (direct
    dependents instead of score_all's transitive BFS count) and silently
    ignored its own `weights` parameter - the same file could score
    differently depending on whether it went through `citygen risk`
    (score_all) or the pre-commit hook / PR bot / `citygen risk --staged`
    (score_paths). Delegating to score_all is the actual task spec
    ("score_all filtered to the given paths") and guarantees both entry
    points always agree.
    """
    buildings = city.get("buildings", [])
    if not buildings:
    ...
    by_path = {b["path"]: i for i, b in enumerate(buildings)}
    by_index = {s["index"]: s for s in score_all(city, weights)}

    res = []
    for p in paths:
        norm_p = p.replace('\\', '/')
        idx = by_path.get(norm_p)
        if idx is None:
            res.append({
                "index": -1,
                "path": p,
                ...
                "blast_known": False,
                "reasons": ["not analysed: not found in city"],
            })
            continue
        res.append(by_index[idx])
